...
test("site exposes attack anatomy and direct GitHub dataset access", () => withServer(async (base) => {
  ...
  assert.doesNotMatch(homepage, /workspace-progress/);
  assert.doesNotMatch(homepage, />Display</);
  ...
  assert.match(appSource, /function tuneHeroPayment\(\)[\s\S]*setTimeout\(\(\) => scoreHeroFeatures\(features\), 280\)/);
